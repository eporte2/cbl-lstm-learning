### DISCLAIMER : The following code is a slightly modified version of McCauley & Christiansen's (2019) openly available code for their Chunk-based Learner (CBL) (see https://github.com/StewartMcCauley/CBL). These modifications have been made in order to keep track of production scores by utterance length and a new flag, --incremental_results, has been implemented in order to optionally test child utterances on the production task incrementally throughout processing when child sentence is encountered (as in original McCauley & Christiansen production task implementation) or test on a random 40% of child utterances not seen during training/learning (60% remaining percent are seen during training) in order to compare to other model architectures.
